...
def navigate_url(
    ...
    eval_js(
        ...
        f"""
(() => {{
  window.location.assign({json.dumps(target, ensure_ascii=False)});
  return true;
}})()
""",
    )
    wait_until(
        lambda: current_url(session) != current and current_url(session) != "about:blank",
...
def parse_credential_file(credential_path: Path) -> LoginCredential:
    ...
    if not email or not password:
        raise ValueError(
            "credential file format invalid. Use email/password keys or first two lines as email/password.",
        )
...
def is_login_form_visible(session: "vibium.browser_sync.VibeSync") -> bool:
    return bool(
        eval_js(
            session,
            """
...
def login_with_credentials(
    ...
    if not clicked:
        return {"ok": False, "reason": "submit_click_failed_or_disabled", "state": state}
...
""",
    ...
    navigate_url(session, login_url)

    if is_recatch_logged_in(current_url(session), expected_host):
        log("already logged in")
    else:
        wait_until(
            lambda: is_login_form_visible(session),
            timeout_sec=10.0,
            interval_sec=0.25,
        )
        ...
        if credential is not None:
            ...
            for attempt in range(1, max_login_attempts + 1):
                ...
                log(f"credential login attempt {attempt}/{max_login_attempts}: {login_result}")
                if not login_result.get("ok"):
                    if attempt == max_login_attempts and not manual_login_fallback:
                ...
                url_after_attempt = current_url(session)
                log(f"credential login attempt {attempt} did not complete: url={url_after_attempt}")
                if attempt < max_login_attempts and wait_until(
                    lambda: is_login_form_visible(session),
                    timeout_sec=6.0,
                    interval_sec=0.25,
                ):
                    log("login form still visible; retrying credential login once more")
                    continue
...
""",
    ...
    if "/leads" not in current_url(session):
        ...
        navigate_url(session, leads_url)
        time.sleep(1.0)
