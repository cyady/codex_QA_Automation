...
def _extract_field_defs(node: Any, out: dict[str, dict[str, Any]]) -> None:
    ...
    if all(k in node for k in ("id", "label", "type", "category")):
        ...
        out[fid] = merged
    # New layout_settings shape can include lightweight field nodes:
    # {"id": "...", "category": "custom", "required": false, "is_visible": true}
    elif "id" in node and "category" in node and "is_visible" in node:
        fid = str(node["id"])
        merged = dict(node)
        if fid in out:
            prev_visible = bool(out[fid].get("is_visible"))
...
def build_effective_schema(
    ...
    custom_field_obj = deal_view.get("deal", {}).get("custom_field") or {}
    custom_ids = {
        str(k)
        for k, v in custom_field_obj.items()
        if v not in (None, "", [], {})
    }
