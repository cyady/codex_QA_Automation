...
(() => {
  ...
  if (wanted) {
    picked =
      candidates.find((item) => normalize(item.text) === wanted)
      || candidates.find((item) => normalize(item.text).includes(wanted))
      || candidates.find((item) => wanted.includes(normalize(item.text)));
  }
...
"""
    ...
    result = eval_js(session, script.replace("__TARGET_TEXT__", target_literal))
    return result if isinstance(result, dict) else {"ok": False, "raw": result}
...
(() => {
  ...
  return [...document.querySelectorAll(".recatch-ant-select-item-option, [role='option']")]
...
def ensure_lead_form_ready(
    ...
    wait_until(
        lambda: bool(_probe_modal_state(session).get("ready")),
        timeout_sec=3.0,
        interval_sec=0.2,
    )
