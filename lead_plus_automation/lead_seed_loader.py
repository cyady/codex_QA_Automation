...
def parse_optional_int(value: str) -> Optional[int]:
    """Convert string to int when possible, otherwise return None."""
    v = (value or "").strip()
    if v == "":
        return None
    return int(v)
...
def parse_bool_with_default(value: str, default: bool) -> bool:
    """Parse boolean string, returning default when value is empty."""
    v = (value or "").strip()
    if v == "":
        return default
    return parse_bool(v)
...
def parse_raw_str(value: str) -> str:
    """Return raw stripped string (can be empty)."""
    return (value or "").strip()


@dataclass(frozen=True)
class LeadSeedRecord:
    """Raw row model that mirrors lead_seed.csv columns."""
    ...
    def from_csv_row(cls, row: dict[str, str]) -> "LeadSeedRecord":
        """Create a typed record from one CSV row."""
        return cls(
...
class LeadFormVariables:
    """Automation-friendly variable set for lead creation.

    This is the object that later lead-create automation can consume directly.
    """
...
def build_form_variables(record: LeadSeedRecord) -> LeadFormVariables:
    """Map one CSV record into automation variables."""
...
def load_lead_seed_records(csv_path: Path) -> list[LeadSeedRecord]:
    """Read all rows from lead_seed.csv as typed records."""
...
def select_records(records: list[LeadSeedRecord], case_id: Optional[str]) -> list[LeadSeedRecord]:
    """Optionally filter records by case_id."""
    if not case_id:
        return [r for r in records if r.automation_enabled]

    selected = [r for r in records if r.case_id == case_id]
    if not selected:
...
def parse_args() -> argparse.Namespace:
    """CLI args for quick local usage."""
    ...
    parser.add_argument(
        ...
        help="Path to lead seed CSV file (default: data/lead_seed.csv)",
    )
    parser.add_argument(
        "--case-id",
        default=None,
        help="Optional case_id filter (example: QA_DYN_001)",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def main() -> int:
    ...
    # Resolve relative path from this script directory.
    ...
    # This is the core output: each selected record mapped into symbolic variables.
    mapped = [build_form_variables(r) for r in selected]

    for idx, variables in enumerate(mapped[: args.limit], start=1):
        print(f"\n--- Variables #{idx} (case_id={variables.case_id}) ---")
